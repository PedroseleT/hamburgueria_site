...
export default function Carrinho() {
  ...
  // Lógica de Endereço (igual da Home)
  const handleBuscarCep = async () => {
    const cepLimpo = cep.replace(/\D/g, "");
    if (cepLimpo.length !== 8) { toast.error("Digite um CEP válido com 8 dígitos."); return; }
    setLoadingCep(true); setAddressData(null);
    try {
      const res = await fetch(`https://viacep.com.br/ws/${cepLimpo}/json/`);
      const data = await res.json();
      if (data.erro) toast.error("CEP não encontrado.");
      else {
        setAddressData({ rua: data.logradouro ?? "", bairro: data.bairro ?? "", cidade: `${data.localidade} - ${data.uf}` });
  ...
  const saveAndClose = () => {
    ...
    if (!addressData || !addressData.rua.trim() || !numero.trim() || !addressData.bairro.trim() || !addressData.cidade.trim()) {
    ...
    setCep(""); setNumero(""); setAddressData(null); setEnderecoEditado(false);
  ...
  return (
    <main className="carrinho-container" style={{ backgroundColor: "#0a0a0a", minHeight: "100vh", color: "#fff", fontFamily: "'Oswald', sans-serif" }}>
      ...
      {/* MODAL DE ADICIONAR ENDEREÇO ESTILIZADO IGUAL A HOME */}
      {showAddressModal && (
        <div style={modalOverlayStyles} onClick={() => setShowAddressModal(false)}>
          <div style={{ ...modalContentStyles, padding: "24px 20px", background: "#0c0c0c", borderRadius: "20px 20px 0 0", position: "absolute", bottom: 0, width: "100%", maxWidth: "none" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ height: 3, background: "linear-gradient(90deg, #b91c1c, #ff6b35, transparent)", position: "absolute", top: 0, left: 0, right: 0 }} />
            <div style={{ display: "flex", justifyContent: "center", padding: "0 0 10px" }}><div style={{ width: 32, height: 3, background: "#222", borderRadius: 2 }} /></div>
            
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ fontSize: 18, fontWeight: 900, textTransform: "uppercase", fontFamily: "Impact, sans-serif", margin: 0, color: "#fff" }}>Taxa de Entrega</h2>
              <button onClick={() => setShowAddressModal(false)} style={{ background: "#161616", border: "1px solid #222", color: "#888", width: 30, height: 30, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><X size={14} /></button>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
              <input type="tel" placeholder="00000-000" maxLength={9} value={cep}
                onChange={e => { const v = e.target.value.replace(/\D/g, "").slice(0, 8); setCep(v.length > 5 ? `${v.slice(0,5)}-${v.slice(5)}` : v); setAddressData(null); }}
                onKeyDown={e => { if (e.key === "Enter") handleBuscarCep(); }}
                style={{ flex: 1, background: "#111", border: "1px solid #262626", borderRadius: 8, padding: "12px", color: "#fff", fontSize: 16, outline: "none", fontFamily: "sans-serif", letterSpacing: "0.05em" }} />
              <button onClick={handleBuscarCep} disabled={loadingCep} style={{ background: "#b91c1c", border: "none", color: "#fff", padding: "12px 20px", borderRadius: 8, fontWeight: 900, fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", opacity: loadingCep ? 0.7 : 1 }}>
                {loadingCep ? <Loader2 className="animate-spin" size={16} /> : "BUSCAR"}
              </button>
            </div>

            {addressData && (
              <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 12, padding: 16, marginTop: 20 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 4 }}>
                  <div style={{ flex: 1 }}>
                    <input placeholder="Rua / Avenida" value={addressData.rua} onChange={e => { setAddressData({ ...addressData, rua: e.target.value }); setEnderecoEditado(true); }}
                      style={{ width: "100%", background: "#0a0a0a", border: `1px solid ${enderecoEditado && !addressData.rua.trim() ? "#b91c1c" : "#262626"}`, borderRadius: 8, padding: "10px 12px", color: "#fff", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
                  </div>
                  <div style={{ width: 70 }}>
                    <input placeholder="Nº" value={numero} onChange={e => { setNumero(e.target.value); setEnderecoEditado(true); }}
                      style={{ width: "100%", background: "#0a0a0a", border: `1px solid ${enderecoEditado && !numero.trim() ? "#b91c1c" : "#262626"}`, borderRadius: 8, padding: "10px 12px", color: "#fff", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
                  </div>
                </div>
                
                <div style={{ marginBottom: 4 }}>
                  <input placeholder="Bairro" value={addressData.bairro} onChange={e => { setAddressData({ ...addressData, bairro: e.target.value }); setEnderecoEditado(true); }}
                    style={{ width: "100%", background: "#0a0a0a", border: "1px solid #262626", borderRadius: 8, padding: "10px 12px", color: "#fff", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
                </div>
                
                <div style={{ marginBottom: 16 }}>
                  <input placeholder="Cidade - UF" value={addressData.cidade} onChange={e => { setAddressData({ ...addressData, cidade: e.target.value }); setEnderecoEditado(true); }}
                    style={{ width: "100%", background: "#0a0a0a", border: "1px solid #262626", borderRadius: 8, padding: "10px 12px", color: "#fff", fontSize: 13, outline: "none", boxSizing: "border-box" }} />
                </div>
...
const modalOverlayStyles: React.CSSProperties = { position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'flex-end', backdropFilter: 'blur(8px)' };
